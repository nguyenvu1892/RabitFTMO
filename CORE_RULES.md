# 📜 CORE_RULES.md - Dự án Rabit_FTMO AI (XAGUSD)
...
## 2. Quản trị rủi ro FTMO (FTMO Normal Rules)
* **Loại tài khoản:** FTMO Normal.
* **Quy tắc Intraday (Day Trading):** Bắt buộc có module `Force_Close`. Tất cả các vị thế XAGUSD mở phải được tự động thanh lý trước giờ đóng cửa phiên giao dịch hằng ngày, và dọn sạch lệnh trước 2 tiếng ngày thứ Sáu để tránh phí qua đêm và gap đầu tuần.
* **News Filter (Bộ lọc tin tức):** Tích hợp API Lịch kinh tế. Tự động khóa giao dịch (Pause Trading) trước và sau 30 phút khi có tin tức Đỏ (High Impact) của đồng USD.
...
* **M5 (The Trigger):** Khung thời gian thực thi lệnh. Bot chỉ kích hoạt quét tín hiệu M5 khi giá XAGUSD đã tiến vào vùng POI của M15/H1.
...
3.  **Cấu trúc nến Pinbar (M5):** Tỷ lệ râu nến/thân nến phải đạt chuẩn do người dùng định nghĩa để xác nhận sự từ chối giá tại POI.
...
* **Kiến trúc Modular (OOP):** Tách biệt rõ ràng các file logic: `data_pipeline.py` (kéo dữ liệu), `strategy_engine.py` (tính toán 5 vũ khí), `risk_manager.py` (quản lý quỹ FTMO), `execution.py` (gửi lệnh) và `ml_model.py` (mô hình AI).
